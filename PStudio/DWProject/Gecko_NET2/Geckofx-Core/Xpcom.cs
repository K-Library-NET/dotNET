...
namespace Gecko
{
	...
	public static class Xpcom
	{
		...
		/// </summary>
        public static event BaseTypes.Action BeforeInitalization;

		/// <summary>
		/// Occurs once after XpCom has been initalized.
		/// </summary>
        public static event BaseTypes.Action AfterInitalization;
		...
		public static TInterfaceType CreateInstance<TInterfaceType>(string contractID)
		{
			...
			return (TInterfaceType)GetObjectForIUnknown(ptr);
		}
		...
		public static object QueryInterface(object obj, Guid iid)
		{
			...
			if (ppv == IntPtr.Zero)
			{
				...
				if (pInterfaceRequestor != IntPtr.Zero)
				{
					...
					QI_nsIInterfaceRequestor req = (QI_nsIInterfaceRequestor)GetObjectForIUnknown(pInterfaceRequestor);
		...
		internal static int FreeComObject<T>( ref T obj )
			where T : class
		{
#if false
	// When debug -> use special version, that writes debug information
			...
			var localObj = Interlocked.Exchange( ref obj, null );
			// if it is already null -> return
			if ( localObj == null ) return 0;
			return Marshal.ReleaseComObject( localObj );
#endif
		}
