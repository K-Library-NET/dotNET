        ...
        : Control
    {
        ...
        protected override void OnHandleDestroyed(EventArgs e)
        {
            if (_target != null)
            {
                ...
                int count = Marshal.ReleaseComObject(_target);
            }
                ...
                switch (m.Msg)
                {
                    ...
                    case WM_MOUSEACTIVATE:
                        ...
                        if (Xpcom.IsWindows)
                        {
                            ...
                            {
                                var str = string.Format("+WM_MOUSEACTIVATE lastfocus={0:X8}", focus.ToInt32());
                            ...
                            else
                            {
                                var str = string.Format("-WM_MOUSEACTIVATE lastfocus={0:X8} ", focus.ToInt32());
                                System.Diagnostics.Debug.WriteLine(str);
                                str = string.Format("is_focused={0} ", _isFocused);
                                System.Diagnostics.Debug.WriteLine(str);
                                if (!_isFocused)
                                {
                                    if (WebBrowserFocus != null)
        ...
        public void UserInterfaceThreadInvoke(BaseTypes.Action action)
        {
            if (InvokeRequired)
            {
                Invoke(new BaseTypes.Action(() => SafeAction(action)));
            }
            else
            {
                SafeAction(action);
        ...
        /// </summary>
        /// <param name="action"></param>
        private void SafeAction(BaseTypes.Action action)
        {
            try
            {
                action();
        ...
        /// <returns></returns>
        public T UserInterfaceThreadInvoke<T>(BaseTypes.Func<T> func)
        {
            if (InvokeRequired)
            {
                return (T)Invoke(new BaseTypes.Func<T>(() => SafeFunc(func)));
            }
        ...
        /// <param name="func"></param>
        /// <returns></returns>
        private T SafeFunc<T>(BaseTypes.Func<T> func)
        {
            T ret = default(T);
            try
            {
