    ...
    //nsIWindowProvider,
    {
        ...
        public Uri Url
        {
            get
            {
                ...
                var uri = Xpcom.ToUri(locationComObject);
        ...
        public GeckoWindow Window
        {
            get
            {
                ...
                return _Window ?? (_Window = ExtensionMethods.Wrap(WebBrowser.GetContentDOMWindowAttribute(),
                     GeckoWindow.Create));
        ...
        public GeckoDomDocument DomDocument
        {
            get
            {
                ...
                var ret = ExtensionMethods.Wrap(domWindow.GetDocumentAttribute(),
                     GeckoDomDocument.CreateDomDocumentWraper);
                //var ret = domWindow.GetDocumentAttribute().Wrap(GeckoDomDocument.CreateDomDocumentWraper);
        ...
        void nsIContextMenuListener2.OnShowContextMenu(uint aContextFlags, nsIContextMenuInfo info)
        {
            ...
            try
            {
                ...
                backgroundImageSrc = Xpcom.ToUri(src);
                //backgroundImageSrc = src.ToUri();
                Xpcom.FreeComObject(ref src);
            }
            catch (COMException comException)
            {
                if ((comException.ErrorCode & 0xFFFFFFFF) != 0x80004005)
...
#warning when geckoFX is used in WPF application there are no forms. We should rewrite this code
        ...
        void nsIWebProgressListener.OnStateChange(nsIWebProgress aWebProgress, nsIRequest aRequest, uint aStateFlags, int aStatus)
        {
            ...
            var request = Gecko.Net.Request.WrapRequest(aRequest);

            #region request parameters
            Uri destUri = null;
            Uri.TryCreate(request.Name, UriKind.Absolute, out destUri);
            var domWindow = ExtensionMethods.Wrap(aWebProgress.GetDOMWindowAttribute(),
                 GeckoWindow.Create);
